...
```{r}
p <- ggplot(sum_dwelling_type_by_year, aes(DWELLING_TYPE, total_consumption_GWh)) +  theme(axis.text.x = element_text(size=5))
p <-  p + geom_boxplot()
...
ggplot(df_filtered, aes(x = year, y = consumption_GWh, group = month, color = month)) +
  ...
  theme_minimal() +
  labs(x = "Year", y = "Consumption (GWh)", title = "3-Room Electricity Consumption Over The Years", color = 'Month') +
